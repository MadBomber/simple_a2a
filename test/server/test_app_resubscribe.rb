...
class TestServerAppResubscribe < Minitest::Test
  ...
  def test_resubscribe_terminal_task_returns_unsupported_operation
    ...
    resub_body = JSON.generate({ "jsonrpc" => "2.0", "id" => 2, "method" => "tasks/resubscribe", "params" => { "id" => task_id } })
    resub_resp = JSON.parse(sync_rack.post("/", input: resub_body, "CONTENT_TYPE" => "application/json").body)
  ...
  def test_resubscribe_first_event_is_task_snapshot
    with_server(build_app.app) do |url, task|
      ...
      loop do break if captured_task_id; task.sleep(0.01) end
